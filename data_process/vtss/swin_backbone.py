...
def global_position_index(
    ...
    frags = torch.stack(torch.meshgrid(frags_d, frags_h, frags_w, indexing="ij")).float()
    coords = (
...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, mask=None, fmask=None, resized_window_size=None):
        ...
        if hasattr(self, "fragment_position_bias_table"):
            fragment_position_bias = self.fragment_position_bias_table[rpi.reshape(-1)].reshape(
                N, N, -1
            )  # Wd*Wh*Ww,Wd*Wh*Ww,nH
        ...
        if fmask is not None:
            ...
            if hasattr(self, "fragment_position_bias_table"):
                relative_position_bias = relative_position_bias * fgate + fragment_position_bias * (
                    1 - fgate
                )

            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + relative_position_bias.unsqueeze(
                0
            )
...
class SwinTransformer3D(nn.Module):
    ...
    def load_swin(self, load_path, strict=False):
        ...
        for k in relative_position_bias_table_keys:
            ...
            if isinstance(self.window_size, list):
                ...
                L2 = (2 * self.window_size[i_layer][1] - 1) * (2 * self.window_size[i_layer][2] - 1)
            ...
            if nH1 != nH2:
                ...
                if L1 != L2:
                    ...
                    relative_position_bias_table_pretrained = (
                        relative_position_bias_table_pretrained_resized.transpose(0, 1).view(
                            nH2, 15, L2
                        )
                    )
    ...
    def forward(self, x, multi=False, layer=-1, adaptive_window_size=True):
        ...
        if multi:
            shape = x.shape[2:]
            return torch.cat(
                [F.interpolate(xi, size=shape, mode="trilinear") for xi in feats[:-1]], 1
            )
        elif layer > -1:
