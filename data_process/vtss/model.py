...
from vtss.swin_backbone import SwinTransformer3D as VideoBackbone
from vtss.swin_backbone import swin_3d_tiny, swin_3d_small
from vtss.conv_backbone import convnext_3d_tiny, convnext_3d_small, convnext_tiny
from vtss.head import (
    VQAHead,
    IQAHead,
    VARHead,
    MLPhead,
        ...
        if inference:
                ...
                for key in vclips:
                    ...
                    if key == "feature":
                        ...
                        feat, _ = getattr(self, key.split("_")[0] + "_backbone")(
                            torch.squeeze(vclips[key], dim=2),
                            multi=self.multi,
                            layer=self.layer,
                            **kwargs
                        )
            ...
            for key in vclips:  # fragments、image、feature
                if key == "feature":
                    ...
                    feat, _ = getattr(self, key.split("_")[0] + "_backbone")(
                        torch.squeeze(vclips[key]), multi=self.multi, layer=self.layer, **kwargs
                    )
                elif key == "image":
                    feat = getattr(self, key.split("_")[0] + "_backbone")(
                        torch.squeeze(vclips[key])
                    )
                else:
                    feat = getattr(self, key.split("_")[0] + "_backbone")(
                        vclips[key], multi=self.multi, layer=self.layer, **kwargs
