...
from diffnext.models.builder import build_vae
...
def actor_fn(input_queue, output_queue, stride=2, seqlen=52, resize=384, crop_size=(384, 640)):
    ...
    while True:
        ...
        frame_ids = list(range(st, ed, stride))
...
def dump_batch(batch_ids, batch_vid, anns, writer, vae):
    ...
    for line_id, vid in zip(batch_ids, batch_moments):
        ...
        writer.write({"moments": vid.tobytes(), **example})
    return [], []


def dump_videos(dataset, anns, writer, vae, stride=2, seqlen=49):
    num_readers, lines = 16, open(dataset).readlines()
    # img_args, batch_size = {"resize": 512, "crop_size": (512, 768)}, 1
    # img_args, batch_size = {"resize": 256, "crop_size": (256, 384)}, 8
    # img_args, batch_size = {"resize": 256, "crop_size": (256, 256)}, 8
    # img_args, batch_size = {"resize": 512, "crop_size": (512, 512)}, 2
    img_args, batch_size = {"resize": 480, "crop_size": (480, 768)}, 2
    # img_args, batch_size = {"resize": 320, "crop_size": (320, 512)}, 5
    ...
    for i in range(len(lines)):
        ...
        if frames is None:
            ...
            continue
        if frames.shape != (49, 480, 768, 3):
            # if frames.shape != (33, 320, 512, 3):
