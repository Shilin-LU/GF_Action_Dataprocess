...
from vtss.datasets import get_frames, get_spatial_fragments
...
def actor_fn(input_queue, output_queue, num_frames=9, resize=384):
    ...
    while True:
        ...
        center_st = (ed - st - 64) // 2
        try:
            frames = get_frames(video_path, center_st, center_st + 64)
...
def dump_batch(batch_ids, batch_vid, anns, model):
    ...
    x = np.stack([get_spatial_fragments(vid) for vid in batch_vid])
    ...
    x = model({"fragments": x}, reduce_scores=False)[0].unflatten(0, (bsz, -1))
    batch_vtss = x.flatten(1).float().mean(1).tolist()
    for line_id, vtss in zip(batch_ids, batch_vtss):
        anns[line_id]["vtss"] = vtss
    return [], []
...
if __name__ == "__main__":
    ...
    model = DiViDeAddEvaluator().to(device).to(dtype).eval()
